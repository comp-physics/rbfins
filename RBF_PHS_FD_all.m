...
for m1 = 1:NxNy1  
    ...
    if d == -1  % Pure RBF case (no polynomial augmentation)
        ...
        X = x(:,ones(1,d+1)); X(:,1) = 1; X = cumprod(X,2);  % Powers of x: [1, x, x^2, ...]
        Y = y(:,ones(1,d+1)); Y(:,1) = 1; Y = cumprod(Y,2);  % Powers of y: [1, y, y^2, ...]
        ...
        if d >= 2  % Quadratic terms: derivatives of x^2, xy, y^2  
            L1(4,3) = 2; L1(4,4) = 2;  % Laplacian of x^2: d^2/dx^2(x^2) = 2, d^2/dy^2(x^2) = 0 -> sum = 2
            L1(6,3) = 2; L1(6,5) = 2;  % Laplacian of y^2: d^2/dx^2(y^2) = 0, d^2/dy^2(y^2) = 2 -> sum = 2
