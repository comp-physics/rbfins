function Vops = build_velocity_operators(G, xy, xy1, boundary_y, boundary_out, S, cfg, distances)
  ...
  D = RBF_PHS_FD_all(boundary_y, xy1, Nearest_Idx_b_y, cfg.rbf.order_boundary, ...
  ...
  Nearest_Idx_b_out = [(length(xy) + length(boundary_y) + 1: ...
                        (length(xy) + length(boundary_y) + length(boundary_out)))', Nearest_Idx_b_out];

  D = RBF_PHS_FD_all(boundary_out, xy1, Nearest_Idx_b_out, cfg.rbf.order_boundary, ...
                     cfg.rbf.poly_degree_boundary, cfg.rbf.derivative_order);
  Vops.Dx_b_0 = D{1};
  ...
  D_all = RBF_PHS_FD_all(xy1, xy1, S.Nearest_Idx, cfg.rbf.order_main, ...
                         cfg.rbf.poly_degree_main, cfg.rbf.laplacian_order);
  ...
  Nearest_Idx_nb = xy(:, 1) > x_max - x_max_dist | xy(:, 1) < x_min + x_min_dist * 2 | ...
                   xy(:, 2) > y_max - y_max_dist | xy(:, 2) < y_min + y_min_dist;
  Idx = (1:length(xy))';
