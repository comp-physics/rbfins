function P = build_pressure_system(G, xy_s, xy1_s, cfg)
    ...
    [Nearest_Idx_b_y] = nearest_interp(boundary_y_s, ...
                                       [xy_s; xy1_s(length(xy_s) + length(boundary_y_s) + 1, :)], ...
                                       cfg.rbf.stencil_size_boundary_wall);
    Nearest_Idx_b_y = [(length(xy_s) + 1:length(xy_s) + length(boundary_y_s))', Nearest_Idx_b_y];

    D = RBF_PHS_FD_all(boundary_y_s, xy1_s, Nearest_Idx_b_y, cfg.rbf.order_boundary, ...
                       cfg.rbf.poly_degree_boundary, cfg.rbf.derivative_order);
    Dy_b_s = D{2}; % y-derivative operator for wall boundary condition

    % Inlet boundary: Neumann BC (dp/dx = 0)
    [Nearest_Idx_b_x] = nearest_interp(boundary_in_s, ...
                                       xy1_s(1:length(xy_s) + length(boundary_y_s), :), ...
                                       cfg.rbf.stencil_size_boundary_wall);
    Nearest_Idx_b_x = [(length(xy1_s) + 1 - length(boundary_obs_s) - length(boundary_in_s): ...
                        length(xy1_s) - length(boundary_obs_s))', Nearest_Idx_b_x];

    D = RBF_PHS_FD_all(boundary_in_s, xy1_s, Nearest_Idx_b_x, cfg.rbf.order_boundary, ...
                       cfg.rbf.poly_degree_boundary, cfg.rbf.derivative_order);
    Dx_in_s = D{1}; % x-derivative operator for inlet boundary condition

    % Outlet boundary: Neumann BC (dp/dx = 0)
    [Nearest_Idx_b_x] = nearest_interp(boundary_out_s, ...
                                       xy1_s(1:length(xy_s) + length(boundary_y_s), :), ...
                                       cfg.rbf.stencil_size_boundary_outlet);
    Nearest_Idx_b_x = [(length(xy_s) + 1 + length(boundary_y_s): ...
                        length(xy_s) + length(boundary_y_s) + length(boundary_out_s))', Nearest_Idx_b_x];

    D = RBF_PHS_FD_all(boundary_out_s, xy1_s, Nearest_Idx_b_x, cfg.rbf.order_boundary, ...
                       cfg.rbf.poly_degree_boundary, cfg.rbf.derivative_order);
    ...
    L1 = [L1, [ones(length(xy_s), 1); zeros(length(boundary_s), 1)]; ...
          [ones(1, length(xy_s)), zeros(1, length(boundary_s))], 0];
