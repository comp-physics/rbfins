function [W3, p] = NS_2d_fractional_step_PHS(dt, nu, W1, W2, Dy, Dx, L_inv, L_u_inv, L_v_inv, ...
  ...
  if cfg.logging.trace_substeps
    fprintf('[substep] advection: max|H_u|=%.3e, max|H_v|=%.3e\n', max(abs(H_U)), max(abs(H_V)));
    if any(~isfinite(U1)) || any(~isfinite(V1))
      warning('%s', '[substep] advection produced non-finite values');
    end
  end
  ...
  if cfg.logging.trace_substeps
    fprintf('[substep] diffusion: max|u**|=%.3e, max|v**|=%.3e\n', max(abs(U2)), max(abs(V2)));
    if any(~isfinite(U2)) || any(~isfinite(V2))
      warning('%s', '[substep] diffusion produced non-finite values');
    end
  end
  ...
  if cfg.logging.trace_substeps
    div_l2 = norm(F0, 2);
    fprintf('[substep] pressure: div(u**)=%.3e (L2)\n', div_l2);
    if any(~isfinite(p))
      warning('%s', '[substep] pressure solve produced non-finite values');
    end
  end
  ...
  if cfg.logging.trace_substeps
    fprintf('[substep] correction: max|u|=%.3e, max|v|=%.3e\n', max(abs(U3)), max(abs(V3)));
    if any(~isfinite(U3)) || any(~isfinite(V3))
      warning('%s', '[substep] correction produced non-finite values');
    end
  end
