function [p,t]=distmesh2d_robust(fd,fh,h0,bbox,pfix,varargin)
%DISTMESH2D_ROBUST 2-D Mesh Generator with iteration limits for rectangles
%
% Based on distmesh2d by Per-Olof Persson but with modified convergence
% tolerances and maximum iteration limits to prevent hanging on sharp corners.

% Relaxed tolerances and iteration limits
dptol=.05; ttol=.5; Fscale=1.2; deltat=.2; geps=.001*h0; deps=sqrt(eps)*h0;
densityctrlfreq=30;
maxiter=1000; % Maximum iterations to prevent hanging

% 1. Create initial distribution in bounding box (equilateral triangles)
% Use coarser initial grid for rectangles to improve convergence
h0_fine = h0 * 1.0;
[x,y]=meshgrid(bbox(1,1):h0_fine:bbox(2,1),bbox(1,2):h0_fine*sqrt(3)/2:bbox(2,2));
x(2:2:end,:)=x(2:2:end,:)+h0_fine/2;                      % Shift even rows
